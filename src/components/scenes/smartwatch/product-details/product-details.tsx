...
import { Heart, Truck, Package, Award, Monitor, Cpu, Battery, Star, Send, ImagePlus, X } from "lucide-react";
...
export function ProductDetails({ productId }: ProductDetailsProps) {
    ...
    fetch("/api/smartwatch_api_77_unique.json")
      ...
      .then((data) => {
        if (data.success && data.products) {
          ...
          const foundProduct = watchesWithSpecs.find(
            (p: Smartwatch) => p.id === parseInt(productId)
          );
          
          if (foundProduct) {
            setProduct(foundProduct);
            
            const related = watchesWithSpecs
              .filter((p: Smartwatch) => 
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/smartwatches">
            <Button>Back to Smart Watches</Button>
          </Link>
        </div>
      </div>
    );
  ...
  const images = [
    ...
    product.image, // Side view (we'll use CSS to differentiate)
    product.image, // Back view (we'll use CSS to differentiate)
  ];
    ...
    <section className="bg-white py-8">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex items-center gap-2 text-sm mb-8 text-gray-500">
          ...
          <span>/</span>
          <Link href="/smartwatches" className="hover:text-black">
            Smart Watches
          </Link>
          <span>/</span>
          ...
          <div className="space-y-6">
                  ...
                  <Button
                    ...
                    className={cn(
                      ...
                      selectedStorage === storage
                        ...
                        : "border-gray-300 cursor-pointer bg-white text-gray-700 hover:border-gray-400 hover:text-white"
                    )}
                  >
                    {storage}
                  </Button>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <p className="text-sm font-semibold">1.9"</p>
              </div>
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-sm font-semibold">{product.displayType}</p>
              </div>
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="flex items-center gap-2 mb-2">
                  <Monitor className="w-5 h-5 text-gray-600" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {relatedProducts.map((relatedProduct) => (
                <Link
                  ...
                  href={`/smartwatches/${relatedProduct.id}`}
                  className="group"
                >
